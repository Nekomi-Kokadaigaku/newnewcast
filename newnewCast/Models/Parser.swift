...
class Parser {
  ...
  func getPodcastMetaData(_ APIData: Data) -> [Episodes] {
    ...
    podcastDescription =
      xml["rss"]["channel"]["itunes:summary"].element?.text ?? xml["rss"][
        "channel"
      ]["itunes:subtitle"].element?.text ?? ""
    ...
    for item in xml["rss"]["channel"]["item"].all {
      ...
      if item["itunes:subtitle"].element == nil
        || item["itunes:subtitle"].element?.text == ""
      {
        episode.podcastDescription = item["description"].element?.text ?? ""
      }
      episode.audioURL =
        item["enclosure"].element?.attribute(by: "url")?.text ?? ""
      episode.episodeDuration = item["itunes:duration"].element?.text ?? ""
      let date = Episodes.formatter.date(
        from: item["pubDate"].element?.text ?? ""
      )
