...
open class ScrollingTextView: NSView {
  ...
  open func setup(string: String) {
    ...
    stringSize =
      text?.size(withAttributes: textFontAttributes)
      ?? NSSize(width: 0, height: 0)
    setNeedsDisplay(
      NSRect(x: 0, y: 0, width: frame.width, height: frame.height)
    )
    updateTraits()
  }
}
...
extension ScrollingTextView {
  fileprivate func setSpeed(newInterval: TimeInterval) {
    clearTimer()
  ...
  @objc
  fileprivate func update(_ sender: Timer) {
    point.x = point.x - 1
    setNeedsDisplay(
      NSRect(x: 0, y: 0, width: frame.width, height: frame.height)
    )
