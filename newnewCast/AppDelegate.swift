...
class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  let invisibleWindow = NSWindow(
    contentRect: NSMakeRect(0, 0, 20, 1),
    styleMask: .borderless,
    backing: .buffered,
    defer: false
  )

  private var playerController: NSWindowController? =
  ...
  func applicationDidFinishLaunching(_ aNotification: Notification) {
    ...
    statusItem.button?.sendAction(on: [
      NSEvent.EventTypeMask.leftMouseUp, NSEvent.EventTypeMask.rightMouseUp,
    ])
    statusItem.button?.action = #selector(
      AppDelegate.statusBarButtonClicked(_:)
    )
    ...
    playerController?.window?.level = NSWindow.Level(
    ...
    if event.type == NSEvent.EventType.leftMouseUp {
      if playerController?.window?.isVisible == true {
        NotificationCenter.default.post(
          name: NSNotification.Name(rawValue: "close"),
          object: nil
        )
      ...
      menu.addItem(
        NSMenuItem(
          title: "Newcast version \(appVersion ?? "")",
          action: nil,
          keyEquivalent: ""
        )
      )
  ...
  @IBAction func findPodcastClicked(_ sender: Any) {
    NotificationCenter.default.post(
      name: NSNotification.Name(rawValue: "searchSavedPodcast"),
      object: nil
    )
  }

  func displayPopUp() {
    let rectWindow = statusItem.button?.window?.convertToScreen(
      (statusItem.button?.frame)!
    )
    let menubarHeight = rectWindow?.height ?? 22
    let height = playerController?.window?.frame.height ?? 300
    let xOffset =
      ((playerController?.window?.contentView?.frame.midX)!
        - (statusItem.button?.frame.midX)!)
    let x = (rectWindow?.origin.x)! - xOffset
    xWidth = x
    let y = (rectWindow?.origin.y)!
//    yHeight = y - height + menubarHeight - 65
    playerController?.window?.setFrameOrigin(
      NSPoint(x: x, y: y + menubarHeight - height)
    )
