...
class StatusBarVC: NSViewController {
  ...
  lazy var alertView: NSWindowController? =
    NSStoryboard(name: "Main", bundle: nil).instantiateController(
      withIdentifier: "alertWindowController"
  ...
  override func viewDidLoad() {
    ...
    NotificationCenter.default.addObserver(
      self,
      selector: #selector(setBackgroundImage),
  ...
  func setupUI() {
    ...
    let area = NSTrackingArea.init(
      rect: backgroundImageView.bounds,
  ...
  override func viewDidAppear() {
    if playingIndex == nil {
      scrollingTextViewAuthor.setup(
        string: "                 No Podcast Playing"
      )
    ...
    if currentSelectedPodcastIndex != nil {
      if playingIndex != nil {
        backgroundImageView.sd_setImage(
          with: URL(string: podcastsImageURL[currentSelectedPodcastIndex]),
        ...
        scrollingTextViewEpisode.setup(
          string: "\(episodeTitles[episodeSelectedIndex])"
        )
        scrollingTextViewAuthor.setup(string: "")
        scrollingTextViewEpisode.speed = 5
        view.addSubview(scrollingTextViewEpisode)
