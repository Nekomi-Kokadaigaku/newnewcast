...
class PodcastSearchVC: NSViewController {
  ...
  override func viewDidLoad() {
    ...
    NotificationCenter.default.addObserver(
      ...
      name: NSNotification.Name(rawValue: "updateSearchUI"),
      object: nil
    )
  ...
  @IBAction func searchPodcast(_ sender: Any) {
    ...
    let editedURL = podcastSearchField.stringValue.addingPercentEncoding(
      withAllowedCharacters: NSCharacterSet.urlQueryAllowed
    )!
    let url = URL(
      string:
        "https://itunes.apple.com/search?term=\(editedURL)&entity=podcast&limit=100"
    )
    //
  ...
  @IBAction func addURLButtonClicked(_ sender: Any) {
    ...
    if customURLField.stringValue.count == 0 {
      ...
      customPodcastURL = customURLField.stringValue.addingPercentEncoding(
        withAllowedCharacters: NSCharacterSet.urlQueryAllowed
      )!
...
extension PodcastSearchVC: NSCollectionViewDelegate, NSCollectionViewDataSource,
  NSCollectionViewDelegateFlowLayout
{
  ...
  func collectionView(
    ...
    forecastCell.configurePodcastSearchCell(
      podcastCell: self.podcastSearch[indexPath.item]
    )
