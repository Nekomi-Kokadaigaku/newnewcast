...
class PodcastVC: NSViewController {
  ...
  override func viewDidLoad() {
    ...
    if UserDefaults.standard.array(forKey: "podcasts") == nil {
      ...
      podcastsImageURL =
        UserDefaults.standard.array(forKey: "podcastImagesURL") as? [String]
      podcastsTitle =
        UserDefaults.standard.array(forKey: "podcastsTitle") as? [String]
    }
    ...
    NotificationCenter.default.addObserver(
      self,
      selector: #selector(customURL),
      name: NSNotification.Name(rawValue: "customURL"),
      object: nil
    )
    NotificationCenter.default.addObserver(
      self,
      selector: #selector(addPodcastButtonClicked(_:)),
      name: NSNotification.Name(rawValue: "search"),
      object: nil
    )
  ...
  @objc func savedDetails() {
    NotificationCenter.default.post(
      name: NSNotification.Name(rawValue: "updateTitle"),
      object: nil
    )
  }
    ...
    if searchSavedPodcastsField.stringValue.count != 0 {
      for i in 0..<podcastsTitle.count {
        ...
        if editedTitle.contains(searchSavedPodcastsField.stringValue) {
          ...
          break
        } else if podcasts[i].description.contains(
          searchSavedPodcastsField.stringValue.lowercased()
        ) {
          scroll(position: count)
          break
        }
  ...
  @objc func updateUI() {
    ...
    DispatchQueue.main.async {
      let ctx = NSAnimationContext.current
      ctx.allowsImplicitAnimation = true
      self.collectionView.animator().scrollToItems(
        at: [last as IndexPath],
        scrollPosition: .bottom
      )
      ...
      Timer.scheduledTimer(
        timeInterval: 1.5,
        target: self,
        selector: #selector(self.unhighlight),
        userInfo: nil,
        repeats: false
      )
    }
  ...
  func podcastListing(podcastFeedURL: String) {
    ...
    AF.request(url!).responseData(completionHandler: { (response) in
      ...
      if response.data != nil {
        episodes = parser.getPodcastMetaData(response.data!)
        NotificationCenter.default.post(
          name: NSNotification.Name(rawValue: "updateEpisodes"),
          object: nil
        )
      }
    })
  ...
  @IBAction func addPodcastButtonClicked(_ sender: Any) {
    ...
    let podcastsearch =
      NSStoryboard(name: "Main", bundle: nil).instantiateController(
        withIdentifier: "PodcastSearchVC"
      ) as? NSViewController
  ...
  func highlightItems(selected: Bool, atIndexPaths: Set<NSIndexPath>) {
    if selected == false {
      ...
      NotificationCenter.default.post(
        name: NSNotification.Name(rawValue: "clearPodcastEpisodes"),
        object: nil
      )
    }
  ...
  @IBAction func removePodcastButtonClicked(_ sender: Any) {
    ...
    selectionArray.sort(by: { path1, path2 in
  ...
  func editTimeStamps() {
    ...
    NotificationCenter.default.post(
      name: NSNotification.Name(rawValue: "deletedPodcast"),
      object: nil
    )
  }
  ...
  override func viewWillDisappear() {
    if playingIndex != nil && currentSelectedPodcastIndex != nil {
      ...
      UserDefaults.standard.set(
        currentSelectedPodcastIndex!,
        forKey: "currentSelectedPodcastIndex"
      )
    }
...
extension PodcastVC: NSCollectionViewDelegate, NSCollectionViewDataSource,
  ...
  func collectionView(
    ...
    let forecastItem = collectionView.makeItem(
      withIdentifier: NSUserInterfaceItemIdentifier(
        rawValue: "PodcastCellView"
      ),
      for: indexPath
    )

    if podcastsImageURL.count != 0 {
      guard let forecastCell = forecastItem as? PodcastCellView else {
        return forecastItem
      }
      forecastCell.configurePodcastAddedCell(
        podcastCell: podcastsImageURL[indexPath.item]
      )
      return forecastCell
    } else {
      return forecastItem
  ...
  @IBInspectable var placeHolderColor: NSColor? {
    ...
    set {
      self.placeholderAttributedString = NSAttributedString(
        string: self.placeholderString != nil ? self.placeholderString! : "",
