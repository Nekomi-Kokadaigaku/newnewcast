...
class EpisodeCellView: NSCollectionViewItem {
  ...
  override func viewDidLoad() {
    ...
    NotificationCenter.default.addObserver(
      self,
      selector: #selector(playTestFunction),
      name: NSNotification.Name(rawValue: "playButton"),
      object: nil
    )
    NotificationCenter.default.addObserver(
      self,
      selector: #selector(pauseTestFunction),
      name: NSNotification.Name(rawValue: "pauseButton"),
      object: nil
    )
    NotificationCenter.default.addObserver(
      self,
      selector: #selector(seekToPosition),
    ...
    if UserDefaults.standard.object(forKey: "pausedTimesDictionary") != nil {
      ...
      let decoded =
        UserDefaults.standard.object(forKey: "pausedTimesDictionary") as! Data
      pausedTimesDictionary = try!
        ((NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(decoded as Data)
        as? [Int: [CMTime?]])!)
    ...
    Timer.scheduledTimer(
      timeInterval: 0.05,
      target: self,
      selector: #selector(episodeCheck),
      userInfo: nil,
      repeats: true
    )
  }
  ...
  @objc func episodeCheck() {
    if episodesCheck == 1 {
      if pausedTimesDictionary[podcastSelecetedIndex]?.count ?? -1
        < episodes.count
      {
  ...
  func configureEpisodeCell(episodeCell: Episodes) {
    ...
    if episodeCell.episodeDuration.contains(":") {
      episodePubDateField.stringValue =
        "\(dateFormatter.string(from: episodeCell.pubDate)) • \(episodeCell.episodeDuration)"
    } else {
      if episodeCell.episodeDuration.count != 0 {
        if Double(episodeCell.episodeDuration)! >= 3600 {
          duration =
            String(Int(Double(episodeCell.episodeDuration)! / 60) / 60) + ":"
            + String(
              format: "%02d",
              Int(Double(episodeCell.episodeDuration)! / 60) % 60
            ) + ":"
            + String(
              format: "%02d",
              Int(
                Double(episodeCell.episodeDuration)!.truncatingRemainder(
                  dividingBy: 60
                )
              )
            )
          episodePubDateField.stringValue =
            "\(dateFormatter.string(from: episodeCell.pubDate)) • \(duration!)"
        } else {
          duration =
            String(Int(Double(episodeCell.episodeDuration)! / 60) % 60) + ":"
            + String(
        ...
        episodePubDateField.stringValue =
          "\(dateFormatter.string(from: episodeCell.pubDate))"
      }
    }
  ...
  func sendNotifications() {
    ...
    NotificationCenter.default.post(
      name: NSNotification.Name(rawValue: "setBackground"),
      object: nil
    )
    NotificationCenter.default.post(
      name: NSNotification.Name(rawValue: "playPausePass"),
      object: nil
    )
    NotificationCenter.default.post(
      name: NSNotification.Name(rawValue: "playPausePassStatus"),
      object: nil
    )
  }
    ...
    if playingIndex != nil {
        ...
        && currentSelectedPodcastIndex == podcastSelecetedIndex
      {
        if playButton.alphaValue == 1 {
          if episodeSelectedIndex != nil {
            player = AVPlayer(
              url: URL(string: episodesURL[episodeSelectedIndex])!
            )
            if pausedTimesDictionary[podcastSelecetedIndex]?[
              ...
              player?.seek(
                to: pausedTimesDictionary[podcastSelecetedIndex]![playingIndex]!
              )
              playPlayer()
            }
          }
      ...
      if playButton.alphaValue == 1 {
        if episodeSelectedIndex != nil {
          ...
          if pausedTimesDictionary[podcastSelecetedIndex]?[episodeSelectedIndex]
            == nil
          {
            playPlayer()
          } else {
  ...
  func updateSlider() {
    ...
    player.addPeriodicTimeObserver(
      ...
      if let duration = player.currentItem?.duration {
        ...
        NotificationCenter.default.post(
          name: NSNotification.Name(rawValue: "moveSlider"),
          object: nil
        )
      }
  ...
  @IBAction func infoButtonClicked(_ sender: Any) {
    ...
    guard
      let vc = storyboard.instantiateController(withIdentifier: "EpisodeInfoVC")
        as? NSViewController
    else { return }
    popoverView.contentViewController = vc
    popoverView.behavior = .transient
    popoverView.show(
      relativeTo: infoButton.bounds,
      of: infoButton,
      preferredEdge: .maxX
    )
  }
